<nav id="js-nav-menu" class="w-auto px-2 pt-6 pb-2 bg-[var(--bg)] shadow hidden lg:hidden text-center">
    <ul class="my-0">
            ...
            <li class="pl-4">
                <a
                title="{{ $page->siteName }} Listen"
                href="/listen"
                class="block mt-0 mb-4 text-md no-underline {{ $page->isActive('/listen') ? 'active text-[var(--link)]' : 'text-[var(--text)] underline decoration-[var(--link)] underline-offset-7 hover:text-[var(--link)] hover:no-underline' }}"
                >Listen</a>
             </li>
